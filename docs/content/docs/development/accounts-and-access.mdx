...
Findings that only ever fire once, such as a signup's origin score or one import's list quality, carry a 30-day expiry and are swept out automatically, so a workspace recovers on its own without anyone touching it.
