...
<Callout type="warn" title="make dev shares its database with make up">
Both pin the same compose project (`-p warmbly`), the same `warmbly_postgres_data` volume and the same `warmbly_dev` database, and `make dev` seeds nine fixture accounts by default, including `admin@warmbly.local` with every platform admin permission and a published password.

Those accounts are your instance's accounts. Seeding therefore spends the first-run exemption permanently: a later `make up` finds a claimed instance, issues no setup link, and the sign-up form refuses you with `403 This server is invite only`. Recover with `warmblyctl user create --email you@example.com --admin`, or start over with `make reset`, which destroys the volumes holding the encryption keys.

Use `make dev SEED=false` on a database you intend to self-host from. Full detail in [first run](/development/first-run/#if-the-instance-already-has-accounts).
</Callout>
...
signing in never depends on mail. See [accounts and access](/development/accounts-and-access/#login-codes).
...
docker compose -p warmbly --profile seed run --rm --build seed       # seed
...
Each migration is a pair of files named `<version>_<name>.up.sql` and `<version>_<name>.down.sql`, where the version is six digits and the name is snake_case. Versions are contiguous, so a new migration takes the next number after the highest one on `main`.

Pick that number against `main`, not against your branch. Two branches opened around the same time will otherwise both claim the same next version, and while each one migrates cleanly on its own, the collision on `main` makes golang-migrate refuse to build its source driver. The backend then exits at boot and restart-loops, taking every service that waits on it with it.
...
It reports duplicate versions, gaps, an `.up.sql` with no `.down.sql`, and filenames golang-migrate cannot parse. `make lint` runs it too, and the Migrations job in CI runs it on every pull request that touches the directory.

If a duplicate does reach `main`, renumber the migration that has **not** been released yet. Deployments in the wild have already recorded the other one in `schema_migrations`, and renumbering that one makes them try to apply it a second time.
...
When `SEED_RICH=true` (set in `docker-compose.yml`, and by `make seed`), also loads:
...
Each target sets the same env you would otherwise export by hand (see the
[configuration reference](/development/configuration/) for what each variable
does, and `deploy/config/env.example` for the bare-binary template). Saving a Go file and
...
`AI_PROVIDER=ollama` needs no key and marks the model free (`AI_FREE`), so AI credits are not charged and the assistant shows a "free model" notice. Local and free-tier models are less capable, but tool calls and the full agent loop work; use `llama3.1` (8B) or `qwen2.5`, not a 1B to 3B model. Leave `AI_PROVIDER` unset to keep AI off. See the [self-hosting guide](/development/deployment-guide/) for the full provider list.
...
## Testing the Warmbly Cloud link

`make poollink-dev` boots two instances side by side so you can walk the self-hoster's path end to end: a prod-like cloud (`DEPLOYMENT_MODE=cloud`, Stripe billing gates on with placeholder keys, email verification through Mailpit, the Sunrise Labs sandbox org seeded into the warmup pool) on `:18301` / `:18401`, and a brand-new self-hosted instance with no accounts on `:18302` / `:18402`, with `WARMBLY_CLOUD_URL` pointing at the cloud. Both use the `make infra` containers with their own databases, so `make dev` keeps working next to them.

The script prints a claim link for the self-hosted instance (`scripts/dev-poollink.sh setup-link` prints a fresh one). Connect a mailbox there with SMTP `localhost:11025` (Mailpit, no auth) and IMAP `localhost:10993` (Dovecot accepts any user with password `sandbox`), then follow Settings > Warmbly Cloud: register on the cloud dashboard, approve the code at `/connect`, and enroll the mailbox. The cloud workspace starts on a free trial, so the 10-mailbox allowance and the upgrade card behave like production; checkout itself needs real Stripe keys (`STRIPE_SECRET_KEY` and friends are read from the environment if set). `make poollink-dev-down` stops everything, `make poollink-dev-reset` also drops the databases. Product behavior is described in the [Warmbly Cloud guide](/guides/warmbly-cloud/).
