...
Resting only makes sense while warmup is running, because warmup traffic is what it recovers on. If warmup stops on a resting mailbox (you switch it off, or the workspace loses warmup when a trial ends or a plan changes), there is no health signal to wait for, so the mailbox returns on its own three days after its last rest stamp rather than staying out indefinitely.

### Putting a resting mailbox back yourself

You do not have to wait. A resting mailbox's notice in the drawer has a **Put back into campaigns** button. It lands in `active` unless warmup is running and still reports the mailbox as `throttled` or worse, in which case it stays resting and the drawer says so: a mailbox that warmup can see is struggling should not send cold mail. To override that, turn warmup off for the mailbox first, or hold it in `reserve` and release it.
