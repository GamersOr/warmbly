...
Import quality is measured across everything a workspace has imported rather than per file, so one clean upload does not erase a bad list that is still stored, and the finding fades as good data outweighs it. Postures are re-derived nightly, so a restriction lifts on its own once the evidence behind it no longer holds. A suspension is the exception: it stays until it is reviewed, because time passing is not evidence that anything changed.
