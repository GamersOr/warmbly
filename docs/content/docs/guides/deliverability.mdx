...
Postures are re-derived nightly, so a restriction lifts on its own once the evidence behind it no longer holds. A suspension is the exception: it stays until it is reviewed, because time passing is not evidence that anything changed.
