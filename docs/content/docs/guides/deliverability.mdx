...
### How a posture is reached, and how it is lifted

Detectors file findings against the workspace, each carrying a weight. The posture is the band the weights add up to, which is why several individually tolerable problems can restrict a workspace that no single one would.

A posture is not permanent, and there are three ways out of one:

- **Findings age out.** A detector that only ever fires once, such as the score of the address a workspace signed up with or the quality of one imported list, files its finding with an expiry. After 30 days it is retired and the score falls on its own.
- **Detectors retract.** A finding that is re-checked on a schedule, such as being part of a cluster of workspaces opened from one address, is withdrawn the moment it stops being true.
- **Review.** A platform operator can read every finding on file, retract any one of them, and set the posture directly. A posture set by review holds against the detectors until an operator lifts it, so a workspace cleared by a human is not re-restricted by the evidence that is still on file.
